...
<div itemprop="address" itemscope itemtype="http://schema.org/PostalAddress">
	...
	<?php if( tribe_get_country( $postId ) ) { //BPNY added hidden class to country span because we know what country is usually?>
		<?php $address_out []= '<span itemprop="addressCountry" class="hidden">' . tribe_get_country( $postId ) . '</span>'; ?>
	<?php } ?>

	<?php if ( count( $address_out ) > 0 ) : ?>
	<?php echo implode( ' ', $address_out ); ?>
		<?php // this is the version with a comma - removing the comma because we're not using country
