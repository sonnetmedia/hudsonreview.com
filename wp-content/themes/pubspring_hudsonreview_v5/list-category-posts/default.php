...
// Show category link:
 if( !empty( $single->post_content ) ) {  }
$lcp_display_output .= "<h2 class='catlink'>" .$this->get_category_link('strong'). "</h2>";
...
foreach ($this->catlist->get_categories_posts() as $single):
    ...
    //BP added to flag articles with content
 if( !empty( $single->post_content) ) {
    $lcp_display_output .= " <span class='inline label label-important metadata small' style='font-size:50%;font-weight:400;color:white;'>Full Text</span>";
    }
    ...
    //Show author
    $lcp_display_output .= '<br /><small>'. $this->get_author($single). '</small>' ;
